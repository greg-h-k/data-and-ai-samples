...
This demo deploys two MSK Serverless clusters across two different regions, along with supporting infrastructure (VPCs, security groups, IAM roles and others) and MSK Replicators in both regions to provide an active-active setup.
...
│                                 │              │                                 │
│  ┌───────────────────────────┐  │              │  ┌───────────────────────────┐  │
│  │    MSK Serverless         │  │  Replicate   │  │    MSK Serverless         │  │
│  │    Cluster A              │◀─┼──────────────┼─▶│    Cluster B              │  │
│  │    (IAM Auth)             │  │              │  │    (IAM Auth)             │  │
│  └───────────────────────────┘  │              │  └───────────────────────────┘  │
│            ▲                    │              │            ▲                    │
│            │                    │              │            │                    │
│  ┌─────────┴─────────────────┐  │              │  ┌─────────┴─────────────────┐  │
│  │  MSK Replicator (B→A)     │  │              │  │  MSK Replicator (A→B)     │  │
│  └───────────────────────────┘  │              │  └───────────────────────────┘  │
│                                 │              │                                 │
│  ┌───────────────────────────┐  │              │  ┌───────────────────────────┐  │
│  │   Conduktor EC2           │  │              │  │   Conduktor EC2           │  │
│  │   (Private + SSM)         │  │              │  │   (Private + SSM)         │  │
│  └───────────────────────────┘  │              │  └───────────────────────────┘  │
│                                 │              │                                 │
│  ┌───────────────────────────┐  │              │  ┌───────────────────────────┐  │
│  │        VPC                │  │              │  │        VPC                │  │
│  │  Private Subnets + NAT    │  │              │  │  Private Subnets + NAT    │  │
│  │  VPC Endpoints (SSM/S3)   │  │              │  │  VPC Endpoints (SSM/S3)   │  │
│  └───────────────────────────┘  │              │  └───────────────────────────┘  │
└─────────────────────────────────┘              └─────────────────────────────────┘
```
...
After running through the sample code and exploring active-active replication, remove all related resources:
...
```bash
# Verify MSK clusters are deleted (replace with your regions)
aws kafka list-clusters-v2 --region <region-a>
aws kafka list-clusters-v2 --region <region-b>

# Verify EC2 instances are terminated
aws ec2 describe-instances --region <region-a> --filters "Name=tag:Name,Values=*conduktor*" --query "Reservations[].Instances[].State.Name"
aws ec2 describe-instances --region <region-b> --filters "Name=tag:Name,Values=*conduktor*" --query "Reservations[].Instances[].State.Name"
``` 